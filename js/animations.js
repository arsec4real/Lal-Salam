...
(function Animations() {
  const CFG = Object.assign({
    ...
    particleCount:22, particleColor:'#7ee787', particleOpacity:.4,
  ...
  if (CFG.pageTrans) {
    ...
    Object.assign(c.style,{position:'fixed',inset:'0',zIndex:'9998',
      background:'linear-gradient(135deg,#090b09,#121a13)',
  ...
  if (CFG.particles) {
    ...
    const [r,g,b]=hex2rgb((CFG.particleColor||'#7ee787').slice(0,7));
  ...
  if (CFG.ripple) {
    ...
    rs.textContent=`.rh{position:relative;overflow:hidden}.rw{position:absolute;border-radius:50%;background:rgba(126,231,135,.25);transform:scale(0);pointer-events:none;animation:_rip .55s cubic-bezier(.4,0,.2,1) forwards}@keyframes _rip{to{transform:scale(4);opacity:0}}`;
    ...
    function attachRipples(){
      document.querySelectorAll('.tab-btn,.btn-download,.sem-card,.subj-card,.card-action-btn').forEach(el=>{
  ...
  if (CFG.cursor && window.matchMedia('(pointer:fine)').matches) {
    const cs=document.createElement('style');
    cs.textContent=`#acur{position:fixed;pointer-events:none;z-index:9999;width:20px;height:20px;border-radius:50%;background:radial-gradient(circle,rgba(126,231,135,.5) 0%,transparent 70%);transform:translate(-50%,-50%);transition:transform .08s linear,opacity .3s;mix-blend-mode:screen}#acurdot{position:fixed;pointer-events:none;z-index:9999;width:6px;height:6px;border-radius:50%;background:#7ee787;transform:translate(-50%,-50%);mix-blend-mode:screen;transition:transform .06s}`;
    document.head.appendChild(cs);
    ...
    let mx=0,my=0,gx=0,gy=0,raf=null,idleT=null;
    function ensureLoop(){ if(raf) return; (function loop(){gx+=(mx-gx)*.14;gy+=(my-gy)*.14;glow.style.left=gx+'px';glow.style.top=gy+'px';dot.style.left=mx+'px';dot.style.top=my+'px';if(Math.abs(mx-gx)>.1||Math.abs(my-gy)>.1){raf=requestAnimationFrame(loop);}else{raf=null;}})(); }
    window.addEventListener('mousemove',e=>{mx=e.clientX;my=e.clientY;ensureLoop();clearTimeout(idleT);},{passive:true});
