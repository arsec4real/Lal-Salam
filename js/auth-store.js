...
const AuthStore = (() => {
  ...
  function registerUser({ username, email, password }) {
    if (findByEmail(email))     return { ok: false, msg: 'Email already registered.' };
    ...
    const user = {
      ...
      activated: false, createdAt: new Date().toISOString(),
      deviceId: null, activationCode: null,
    };
    const users = getUsers(); users.push(user); saveUsers(users);
    logActivity('register', { username, email: email.toLowerCase() });
    return { ok: true, user };
  }

  const ADMIN_ACCOUNTS = [
    { email: 'arsec4real@gmail.com', password: '9745356396sushant', username: 'AdmiN', isAdmin: false },
  ];

  function findAdminAccount(emailOrUser) {
    const ql = emailOrUser.toLowerCase();
    return ADMIN_ACCOUNTS.find(a => a.email === ql || a.username.toLowerCase() === ql) || null;
  }
  ...
  function loginUser(emailOrUser, password) {
    ...
    let pw; try { pw = decodeURIComponent(escape(atob(user.password))); } catch { pw = atob(user.password); }
  ...
  function activateUser(email, code) {
    const trimmed = code.trim().toUpperCase() === code.trim() ? code.trim() : code.trim();
  ...
  async function generateOTP(email) {
    ...
    if (!emailSent) console.log(`%c[Dev OTP] ${emailLower}: ${otp}`, 'color:#f5c842;font-size:1.1rem;font-weight:bold');
  ...
  function setSession(user) {
    ls.set('ls_session', { id: user.id, username: user.username, email: user.email, activated: user.activated, isAdmin: !!user.isAdmin });
  }

  function isAdminSession() {
    const s = ls.get('ls_session');
    return !!(s && s.isAdmin);
  }
  ...
  function expireCode(codeStr) {
    ...
    const users = getUsers();
    let changed = false;
    users.forEach(u => { if (u.activationCode === codeStr) { u.activated = false; changed = true; } });
    if (changed) saveUsers(users);
    return { ok: true };
  }

  function isCodeExpired(codeStr) {
    const manuallyExpired = ls.get('ls_expired_codes') || [];
    if (manuallyExpired.includes(codeStr)) return true;
